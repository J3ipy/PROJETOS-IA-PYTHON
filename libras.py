    ...
    if frame is not None:
        ...
        if hands:
            fingers_situation = {
                ...
                'index+middle+pinky+ring': [0, 1, 1, 1, 1],
            ...
            for hand in hands:
                ...
                if (state == fingers_situation['thumb'] and
                    ...
                    config_text('A')
                if state == fingers_situation['index+middle+pinky+ring']:
                    config_text('B')
                ...
                if (state == fingers_situation['index'] and
                    ...
                    config_text('D')
                if state == fingers_situation['none']:
                    config_text('E')
                if (state == fingers_situation['middle+ring+pinky'] and
                        (calculate_distance(hand_points(hands.index(hand), 'thumb_tip'),
                                            hand_points(hands.index(hand), 'index_pip')) > threshold)):
                    config_text('F')
                if (state == fingers_situation['index'] and
                        (calculate_distance(hand_points(hands.index(hand), 'thumb_tip'),
                ...
                if (state == fingers_situation['thumb+index'] and
                        (calculate_distance(hand_points(hands.index(hand), 'middle_pip'),
                         ...
                         calculate_distance(hand_points(hands.index(hand), 'pinky_pip'),
                                            hand_points(hands.index(hand), 'pinky_mcp')) < threshold)):
                    config_text('L')
                if state == fingers_situation['index+middle+ring']:
                    config_text('M')
                if state == fingers_situation['index+middle']:
                    config_text('N')
                if (state == fingers_situation['thumb'] and
                        (calculate_distance(hand_points(hands.index(hand), 'thumb_tip'),
                         ...
                         calculate_distance(hand_points(hands.index(hand), 'index_mcp'),
                                            hand_points(hands.index(hand), 'pinky_mcp')) < threshold)):
                    config_text('O')
                if state == fingers_situation['index']:
                    config_text('Q')
                ...
                if (state == fingers_situation['index+middle+ring'] and
                        (calculate_distance(hand_points(hands.index(hand), 'thumb_ip'),
                                            hand_points(hands.index(hand), 'pinky_tip')) < threshold)):
                    config_text('W')
